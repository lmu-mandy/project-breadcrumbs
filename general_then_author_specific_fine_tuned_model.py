"""
General then Author Specific Fine Tuned Model v1

File is located at:

    https://colab.research.google.com/drive/1tusxZ4SKJY_sRhl4neXppIJ_i2NDEWf6#scrollTo=0L7CX9FP5Iuy

This notebook uses GPT-2 from simpletransformers to generate fairy tale text. 
For our general then author specific fine tuned model, the model is fine tuned on a general set of 
of fairy tales, excluding fairy tales from authors we will specifically use, then different authors,
depending on user choice, and tested on fairy tale data from that specific author or a general fairy tale. 
...
"""
Preprocess general and author specific fairy tale data
"""
import pandas as pd

# TODO: Grimm and HCA work, just need to get perrault parsed and find regex to parse
ftd_general = pd.read_csv('bfb.txt', sep=r'\s{2,}', engine='python', header=None, names=['Line'])
...
lines_general = ftd_general['Line'].tolist()
...
train_cutoff_general = int(len(lines_general) * 0.8)
...
with open('general_train.txt', 'w') as f:
   for line in lines_general[:-train_cutoff_general]:
       f.writelines(line + '\n')
...
# Create a GPT-2 model that is fine-tuned on general fairy tale data then specific author fairy tale data.
